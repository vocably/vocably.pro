import { configureAnalyzer, fixGrammar } from '@vocably/analyze';
import {
...
export const fixGrammarFunction = async (
  ...
  lastValueFrom(
    of(event).pipe(
      map(extractPayload),
      map(sanitizePayload),
      mergeMap((payload) => fixGrammar(payload)),
      map((result) => {
...
exports.fixGrammar = fixGrammarFunction;
